...
public class DemoControllerTest {
	...
	void testGreeting() throws Exception {
		MockHttpServletRequestBuilder  mockRequestBuilder = 
				MockMvcRequestBuilders.get("/greeting").accept(org.springframework.http.MediaType.APPLICATION_JSON);
	...
	void testUseRepository() throws Exception {
		...
		//응답결과가 응답상태코드200임을 예상한다
		resultActions.andExpect(MockMvcResultMatchers.status().isOk());
		org.hamcrest.Matcher<Integer> matcher;
		ResultMatcher resultMatcher;
		
		//응답결과(matcher)는 {"totalCnt":7} 이다
		matcher = org.hamcrest.CoreMatchers.is(expectedCount);
		//jsonPath를 이용해서 totalCnt 프로퍼티값이 7과 같은지 예상한다
