$(function () {
	...
	$form.submit(function () {
		...
		$.ajax({
			...
			method: 'post',
			data: data,
			//success: function(responseText){	//servlet에서 전달데이터가 json이 아닌 형식으로 넘어왔을때
			//	let jsonObj = JSON.parse(responseText);	//json형태로 parsing
			success: function(jsonObj){	//servlet에서 전달데이터가 json 형식일 경우 parsing할 필요 없음
				if(jsonObj.status == 1){
